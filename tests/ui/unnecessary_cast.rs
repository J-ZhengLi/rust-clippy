...
//         let _: f64 = -(8.0 as f64).exp(); // should suggest `-8.0_f64.exp()` here not to change
// code behavior     }
...
fn issue_12860() {
    ...
    impl<'a> Foo<'a> {
        fn as_ptr(&self) -> *const Foo<'a> {
            self as *const _
        }
    }

    fn cast_chain<'a, 'b>(x: &'a Foo<'a>) -> &'b Foo<'b> {
        unsafe { &*(x as *const Foo<'a> as *const Foo<'b>) }
    }

    fn param_cast<'a, 'b>(x: *const Foo<'a>) -> &'b Foo<'b> {
        unsafe { &*(x as *const Foo<'b>) }
    }

    fn local_with_ty<'a, 'b>(x: &'a Foo<'a>) -> &'b Foo<'b> {
        let y: *const Foo<'a> = x as *const _;
        unsafe { &*(y as *const Foo<'b>) }
    }

    fn local_without_ty<'a, 'b>(x: &'a Foo<'a>) -> &'b Foo<'b> {
        let y = x.as_ptr();
        unsafe { &*(y as *const Foo<'b>) }
    }
}
